...
Tính toán chỉ số AQI từ các chất ô nhiễm
"""

def calculate_sub_index(C, breakpoints):
    """
    Tính sub-index AQI cho một chất
    Args:
        C: Nồng độ chất
        breakpoints: Danh sách breakpoints [(C_lo, C_hi, I_lo, I_hi), ...]
    Returns:
        Sub-index AQI
    """
...
def calculate_aqi(co, no2, o3, pm10, pm25, so2):
    ...
    Returns:
        aqi: Chỉ số AQI
        main_pollutant: Chất ô nhiễm chính
        sub_indices: Dictionary các sub-indices
    """
    # PM2.5 (µg/m³)
    pm25_bp = [
        (0.0, 12.0, 0, 50),
    ...
    if not valid_indices:
        return 0, 'unknown', sub_indices
    
    aqi = max(valid_indices.values())
    main_pollutant = max(valid_indices, key=valid_indices.get)
